...
$parentModule = 'MSConnect'
if (-not $ModuleName) { $ModuleName = 'MSConnect.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\MSConnect'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\MSConnect.psd1"
$prereqHash = @{
