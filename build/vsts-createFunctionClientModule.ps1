...
$parentModule = 'MSApiConnect'
if (-not $ModuleName) { $ModuleName = 'MSApiConnect.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\MSApiConnect'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\MSApiConnect.psd1"
$prereqHash = @{
