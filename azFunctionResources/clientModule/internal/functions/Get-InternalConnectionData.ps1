...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'MSApiConnect.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'MSApiConnect.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'MSApiConnect.Client.ProtectedToken'
