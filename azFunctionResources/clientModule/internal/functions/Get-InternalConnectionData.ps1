...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'MSConnect.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'MSConnect.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'MSConnect.Client.ProtectedToken'
